...
class _DelimitedConverter(BaseModel):
    ...
    def _classify_collection_field(cls, name: str, annotation: Any) -> None:
        ...
        inner = unpack_optional(annotation) if is_optional(annotation) else annotation
        args = get_args(inner)
    ...
    @final
    @classmethod
    def _is_mapping_field(cls, field_name: str | None) -> bool:
        """True if the field is annotated as a delimited mapping on the class model."""
        return field_name is not None and field_name in cls._mapping_fieldnames
